...
import no.ntnu.idata2900.project.backend.services.TripLogService;
...
public class TripLogController {
   ...
   */
  @Operation(
      summary = "Get all trip logs",
      description = "Retrieves a list of all recorded trip logs from the database"
  )
  @ApiResponses(value = {
  ...
  public List<TripLogDto> getAllTripLogs(@PathVariable String registrationMark) {
    logger.info("Received request to get all trip logs for registration "
   ...
   */
  @Operation(
      summary = "Edit trip comments",
      description = "Updates the comments for an existing trip log"
  )
  @ApiResponses(value = {
