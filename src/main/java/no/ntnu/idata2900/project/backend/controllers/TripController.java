...
import no.ntnu.idata2900.project.backend.services.TripService;
...
public class TripController {
   ...
   */
  @Operation(
      summary = "Stop the current trip",
      description = "Ends the current fishing trip and saves it to the database"
  )
  @ApiResponses(value = {
  ...
  /**
   * Endpoint for getting the ID of active trip for the specified registration mark. The ID is only
   * returned if it is active.
   *
   * @param registrationMark The specified registraiton mark
   * @return ID of trip if it is active
   */
  @Operation(
      summary = "Get trip ID if active",
      description = "Gets the ID of the current trip if it is active"
  )
  @ApiResponses(value = {
      @ApiResponse(
        responseCode = "200",
        description = "Active trip found",
  ...
  /**
   * Endpoint for getting data points of the trip with the specified trip ID if it is active.
   *
   * @param tripId The specified trip ID
   * @return Data point for trip if is active
   */
  @Operation(
      summary = "Get current trip data points",
      description = "Gets the current trip data points"
  )
  @ApiResponses(value = {
  })
