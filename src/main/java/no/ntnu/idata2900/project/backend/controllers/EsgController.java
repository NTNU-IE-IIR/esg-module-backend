...
import no.ntnu.idata2900.project.backend.services.EsgService;
...
public class EsgController {
   ...
   * @return ResponseEntity containing the ESG data if found, or a 404 Not Found response if no
   *         data exists
   */
  ...
  public ResponseEntity<EsgDto> esgFromVesselTime(
      @PathVariable String regMark,
    ...
    if (esgDto.isPresent()) {
      ...
      logger.error("ESG time period data for vessel with registration mark {} was not "
                   + "found", regMark);
      response = ResponseEntity.notFound().build();
    }
